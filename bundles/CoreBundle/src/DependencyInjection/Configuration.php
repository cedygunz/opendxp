            ...
            ->children()
                ...
                ->children()
                    ...
                    ->integerNode('shared_max_age')
                        ->info('Shared cache (reverse proxy) TTL in seconds. Set on the response as s-maxage when the HTTP cache scope is active. 0 = OpenDXP does not set Cache-Control headers (delegate to project config). Default: 3600.')
                        ->defaultValue(3600)
                        ->min(0)
                    ->end()
                    ->integerNode('max_age')
                        ->info('Browser cache TTL in seconds. Set on the response as max-age when the HTTP cache scope is active. Default: 0 (browser always revalidates through the proxy).')
                        ->defaultValue(0)
                        ->min(0)
                    ->end()
                    ->arrayNode('elements')
                        ...
                        ->children()
                            ->arrayNode('documents')
                                ...
                                ->children()
                                    ...
                                    ->booleanNode('tag_list')
                                        ->info('Also tag/invalidate "doc_list".')
                                        ->defaultTrue()
                                    ->end()
                                ->end()
                            ->end()
                            ->arrayNode('data_objects')
                                ...
                                ->children()
                                    ->booleanNode('enabled')->defaultTrue()->end()
                                    ->booleanNode('tag_list')
                                        ->info('Also tag/invalidate "obj_class_{className}".')
                                        ->defaultTrue()
                                    ->end()
                                ->end()
                            ->end()
