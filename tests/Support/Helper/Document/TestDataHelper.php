...
namespace OpenDxp\Tests\Support\Helper\Document;
...
use OpenDxp\Model\Document\Editable\Multiselect;
...
class TestDataHelper extends AbstractTestDataHelper
{
    ...
    public function assertImage(PageSnippet $pagesnippet, string $field, int $seed = 1, array $params = []): void
    {
        ...
        $this->assertInstanceOf(\OpenDxp\Model\Asset\Image::class, $value);
    ...
    public function assertVideo(PageSnippet $pagesnippet, string $field, int $seed = 1, array $params = []): void
    {
        ...
        $this->assertInstanceOf(\OpenDxp\Model\Asset\Video::class, $video);

        $expectedVideo = $params['video'];
        $this->assertInstanceOf(\OpenDxp\Model\Asset\Video::class, $expectedVideo);

        $this->assertEquals($expectedVideo->getId(), $video->getId());

        $poster = $editable->getPosterAsset();
        $expectedPoster = $params['poster'];
        $this->assertInstanceOf(\OpenDxp\Model\Asset\Image::class, $poster);
        $this->assertInstanceOf(\OpenDxp\Model\Asset\Image::class, $expectedPoster);
