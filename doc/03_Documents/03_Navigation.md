...
OpenDXP comes with a standard navigation implementation in the form of a Twig extension. 
...
1. Build the navigation: `{% set nav = opendxp_build_nav({active: activeDocument, root: navigationRootDocument}) %}`
2. Render the navigation: `opendxp_render_nav(nav)` or `opendxp_nav_renderer('menu').render(nav)`

> The building step does not necessarily need to happen in the view script. In fact the view helper just forwards the
 `build()` call to the `OpenDxp\Navigation\Builder` service. You can also build the navigation in your controller
...
{% if not document is defined or not document %}
    {% set document = opendxp_document(1) %}
{% endif %}

{# get the document which should be used to start in navigation | default home #}
{% set navStartNode = document.getProperty('navigationRoot') %}
{% if not navStartNode is instanceof('\\OpenDxp\\Model\\Document\\Page') %}
    {% set navStartNode = opendxp_document(1) %}
{% endif %}

{% set mainNavigation = opendxp_build_nav({
    active: document,
    root: navStartNode
}) %}

{# later you can render the navigation #}
{{ opendxp_render_nav(mainNavigation) }}
```
...
```twig
{{ opendxp_nav_renderer('menu').renderMenu(mainNavigation,{
    maxDepth: 1,
...
<div class="my-menu">
    ...
    {{ opendxp_render_nav(mainNavigation, 'menu', 'renderMenu', {
        maxDepth: 1,
        ulClass: 'nav navbar-nav'
    }) }}
</div>
...
<div class="my-breadcrumbs">
    <a href="/">{{ 'Home'|trans }}</a> >
    {{ opendxp_render_nav(mainNavigation, 'breadcrumbs') }}
</div>
```
...
```twig
<div class="my-sidebar-menu">
    {% set sideNav = opendxp_build_nav({active: document, root: navStartNode, htmlMenuPrefix: 'my-nav-'}) %}
    
    {{ opendxp_render_nav(sideNav, 'menu', 'renderMenu', {
...
```twig
{# \OpenDxp\Navigation\Renderer\Menu #}
{% set menuRenderer = opendxp_nav_renderer('menu') %}
...
{% for page in pages %}
    <div class="my-menu-element">
        {{ opendxp_nav_renderer('menu').htmlify(page) | raw }}
    </div>
{% endfor %}
```
...
{% set navigation = opendxp_build_nav({active: document, root: navStartNode}) %}
...
{% set mainNavigation = opendxp_build_nav({active: document, root: navStartNode}) %}
