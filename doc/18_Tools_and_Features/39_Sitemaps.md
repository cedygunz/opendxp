...
OpenDXP includes the [`presta/sitemap-bundle`](https://github.com/prestaconcept/PrestaSitemapBundle) which adds a simple,
...
If configured, OpenDXP will set the domain configured as main domain in system settings as default host. Those parameters
...
* [`UrlGenerator`](https://github.com/open-dxp/opendxp/blob/1.x/bundles/SeoBundle/src/Sitemap/UrlGenerator.php)


## Sitemap Generators

OpenDXP adds a way to hook one or more generators into the sitemap generation process. Such generators can be registered,
...
When the sitemap bundle fires its `SitemapPopulateEvent::class` event, OpenDXP will iterate through every
...
opendxp_seo:
    sitemaps:
        generators:
            ...
            # OpenDXP ships a default document tree generator which is enabled by default
...
For more advanced use cases involving OpenDXP models, OpenDXP defines an `AbstractElementGenerator` which is extendable
...
generator to query for this information). For example the [PropertiesFilter](https://github.com/open-dxp/opendxp/blob/1.x/bundles/SeoBundle/src/Sitemap/Element/Filter/PropertiesFilter.php)
excludes elements with a property `sitemaps_exclude`. A **processor** can enhance an entry before it is added to the container. For example, the [ModificationDateProcessor](https://github.com/open-dxp/opendxp/blob/1.x/bundles/SeoBundle/src/Sitemap/Element/Processor/ModificationDateProcessor.php)
...
can change and enhance the behavior of the generator. OpenDXP defines a set of standard implementations which are defined
as service and can directly be consumed.

| Filter                                               | Description                                                                                                                                                                                                                                                                                                        |
|------------------------------------------------------|--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| `OpenDxp\Bundle\SeoBundle\Sitemap\Element\Filter\PropertiesFilter`    | Excludes elements with the boolean property `sitemaps_exclude` set to true and excludes children handling of elements with the boolean property `sitemaps_exclude_children` set to true.                                                                                                                           |
| `OpenDxp\Bundle\SeoBundle\Sitemap\Element\Filter\PublishedFilter`     | Excludes unpublished elements.                                                                                                                                                                                                                                                                                     |
| `OpenDxp\Bundle\SeoBundle\Sitemap\Document\Filter\DocumentTypeFilter` | Used by the `DocumentTreeGenerator`. Excludes documents not matching the list of defined types and handles children only for defined types.                                                                                                                                                                        |
| `OpenDxp\Bundle\SeoBundle\Sitemap\Document\Filter\SiteRootFilter`     | Used by the `DocumentTreeGenerator`. Excludes documents which are root documents of a site when the currently processed site doesn't match the document. E.g. if a document is a site root and the main site is currently processed, it will be excluded for the main site, but later be used for the actual site. |

| Processor                                                     | Description                                                                                                                                                                     |
|---------------------------------------------------------------|---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| `OpenDxp\Bundle\SeoBundle\Sitemap\Element\Processor\ModificationDateProcessor` | Adds the modification date of an element as `lastmod` property.                                                                                                                 |
| `OpenDxp\Bundle\SeoBundle\Sitemap\Element\Processor\PropertiesProcessor`       | Reads the properties `sitemaps_changefreq` and `sitemaps_priority` if set on the element and adds them to the sitemap entry to easily set those properties on an element level. |


#### The DocumentTreeGenerator

OpenDXP ships a default generator for documents implemented in [`DocumentTreeGenerator`](https://github.com/open-dxp/opendxp/blob/1.x/bundles/SeoBundle/src/Sitemap/Document/DocumentTreeGenerator.php).
...
[sitemaps.yaml in the CoreBundle](https://github.com/open-dxp/opendxp/blob/1.x/bundles/CoreBundle/config/sitemaps.yaml).


#### Creating a custom generator

To create your own generator, start by implementing the `GeneratorInterface`. In this section we'll extend the
`AbstractElementGenerator` to create entries for OpenDXP models. A generator to add `BlogArticle` entries to the sitemap
...
use OpenDxp\Model\DataObject\BlogArticle;
...
> It's important that your link generator is able to generate an absolute URL for the given object. Above is only an example.
...
To generate absolute URLs, OpenDXP defines an [url generator](https://github.com/open-dxp/opendxp/blob/1.x/bundles/SeoBundle/src/Sitemap/UrlGenerator.php) which, given a path, takes care of creating an absolute URL
based on the [Request Context](https://symfony.com/doc/current/routing.html#generating-urls-in-commands).

As example how to use the URL generator in a processor:
