...
OpenDXP allows to dynamically define custom element icons & tooltips in the element tree. In addition, the icon of the editor tab can
be changed.
...
```yaml
  App\EventListener\AdminStyleListener:
    tags:
      - { name: kernel.event_listener, event: opendxp.admin.resolve.elementAdminStyle, method: onResolveElementAdminStyle }
```
...
{
    public function onResolveElementAdminStyle(\OpenDxp\Bundle\AdminBundle\Event\ElementAdminStyleEvent $event): void
    {
        ...
        if ($element instanceof \App\Model\Product\Car) {
            DataObject\Service::useInheritedValues(true, function () use ($element) {
                if ($element->getObjectType() == 'actual-car') {
                    ...
                    $this->elementIcon = '/bundles/opendxpadmin/img/twemoji/1f697.svg';
                }
            });
        }
    }
    ...
    public function __construct(ElementInterface $element)
    {
        ...
        if ($element instanceof Asset\Image) {
            if (strpos($element->getKey(), 'C') === 0) {
                ...
                $this->elementIcon = '/bundles/opendxpadmin/img/twemoji/1f61c.svg';
            }
