 ...
 * @internal
 *
 * @property \OpenDxp\Model\DataObject\Classificationstore\StoreConfig $model
 */
class Dao extends Model\Dao\AbstractDao
{
    ...
    public function getById(?int $id = null): void
    {
        ...
        $data = $this->db->fetchAssociative(
            sprintf('SELECT * FROM %s WHERE id = ?', self::TABLE_NAME_STORES),
            [$this->model->getId()]
        );
    ...
    public function getByName(?string $name = null): void
    {
        ...
        $data = $this->db->fetchAssociative(
            sprintf('SELECT * FROM %s WHERE name = ?', self::TABLE_NAME_STORES),
            [$name]
        );
